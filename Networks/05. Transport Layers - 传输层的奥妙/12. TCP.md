...
an extra ‘tag’ inserted into the ethernet packet format saying which network the packet belongs to. 
	...
	* 在类型/大小字段之前增加4个字节，前16位0x8100明确地标记“这是一个标记”(没有真正的未标记的包会有0x8100)， 3位是优先级，1位指定帧是否可删除，12位是标记。  
...
### VLANs可以干的事
* 在一条线缆中中继(trunking)多个网络  
Trunking of multiple networks along one cable
	* VLAN的中继技术是指能让连接在不同交换机上的相同VLAN中的主机互通(比如两台交换机都设置有同一VLAN里的计算机)
